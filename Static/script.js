const Url = "http://192.168.56.1:8000/articles/"
...
document.getElementById('dataForm').addEventListener('submit', async function(event) {
    ...
    const data = {
        articleUniqueId: document.getElementById('id').value,
        articleBrandType: document.getElementById('brandType').value,
...
function displayDataList() {
    ...
    savedData.forEach((data, index) => {
        ...
        dataItem.innerHTML = `
            <p><strong>ID:</strong> ${data.articleUniqueId}</p>
            <p><strong>BrandType:</strong> ${data.articleBrandType} </p>
...
function loadData(index) {
    ...
    if (data) {
        document.getElementById("id").value = data.articleUniqueId;
        document.getElementById("brandType").value = data.articleBrandType;
